  ...
  return (
    <div className="min-h-screen bg-[#FBF9F4] py-12 px-6 font-sans">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        {/* Cabeçalho Acolhedor */}
        <div className="border-b border-orange-100/60 pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-stone-500 text-xs md:text-sm font-medium mt-0.5">Conheça as frentes integradas que dão vida à nossa causa social.</p>
          </div>
          <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-100 rounded-xl px-4 py-2 text-xs font-bold text-[#F07147] w-fit shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {projects.map((proj) => (
            <div 
              key={proj.id}
              className="bg-white border border-orange-100/30 rounded-3xl p-6 shadow-sm flex flex-col justify-between space-y-6"
            >
              <div className="space-y-3">
                <span className="text-[10px] font-bold uppercase tracking-wider bg-stone-50 border border-stone-200 text-stone-500 px-2.5 py-0.5 rounded-lg">
                  {proj.area}
                </span>
              ...
              <a 
                href="#"
                className="text-xs font-bold text-[#1E3A5F] hover:text-[#F07147] flex items-center gap-1 transition-colors border-t border-stone-50 pt-4"
              >
                Conhecer histórico de impacto <ArrowUpRight className="w-3.5 h-3.5" />
              </a>
            </div>
          ))}
        </div>
